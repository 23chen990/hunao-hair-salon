...
- **回归**：新增显示回归覆盖库存 0～6、单/双货架且总显示数不重复；既有 `SalonSupplyModelTests` 分别覆盖取货、卸货、消耗和每日重置。聚焦 R1 套件 `26/26` 通过。
...
- **回归**：`SalonProgressSaveTests`、`SalonMobileCheckpointRegressionTests`、扩建货架回滚测试及 Chromium 刷新/失败重试流程通过。真实 Chromium 证据显示部分投入离圈后刷新仍为同一 `Paid/Balance`；完整购买后故意以 `0/3` 失败，点击“再试一次”仍保持 `Paid=180`、`OPEN` 和开店余额。
...
| 同一工作循环 | 购买前 | 购买后 | 变化 |
|---|---:|---:|---:|
| `source → 货架 → 洗发工位` 实测路径 | 17.7834 | 16.5772 | -1.2062 世界单位 |
| 摇杆移动墙钟时间 | 7.1954 秒 | 6.5744 秒 | -0.6210 秒 |
| 卸货等待时间 | 0.000039 秒 | 0.000015 秒 | -0.000024 秒 |
| 卸货次数 | 1 | 1 | 0 |

证据来自 `tools/check-proximity-pad.py` 的真实 Chromium 触控：同一订单策略、同一 844×390 视口、同一共享容量；两次测量都从 `source=12 / carried=0 / rack=0` 的保存检查点开始，使用实际碰撞地图路线和自动拾取/卸货状态。源点到货架的距离变长，扩建入口到洗发工位的回程缩短，合计路径少 `1.2062` 世界单位，卸货次数不变，等待时间均低于 0.0001 秒。该选定几何路线的功能收益已测得；单次墙钟差为 `0.6210` 秒，但受实时触控路线噪声影响，仍不足以证明普遍的玩家时间收益，因此产品层结论标为 **VALUE_NOT_PROVEN**，不外推到所有顾客路径。
...
| 施工/路线入口 | `Builds/ProximityPadEvidence/report.json` | 部分投入刷新、完整购买刷新、购买后失败重试保留扩建、卸货和路线对比通过；错误 `0` |
...
正式移动回归记录：Day1 完成 `3/3`（目标 `3`），结果余额 `515`、满意度 `69`，随后进入 Day2 准备阶段并在刷新后保持余额和满意度；本轮真实浏览器运行共观察 `439` 个遥测帧，最大等待队列 `2`。未购买扩建的失败重试单独运行完成 `0/3`，点击“再试一次”回到 Day1 开店状态，余额和满意度回到开店值；施工脚本另以已购买扩建状态完成 `0/3` 失败，重试后 `Paid=180`、`OPEN` 和余额 `60` 均保留。
