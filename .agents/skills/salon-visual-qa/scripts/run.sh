...
mode="${1:-demo}"
if [[ "$mode" != "demo" && "$mode" != "asset-lab" && "$mode" != "candidate" && "$mode" != "reference" ]]; then
  print -u2 "用法：run.sh [demo|asset-lab|candidate|reference]"
  exit 2
fi
...
if [[ "$mode" == "demo" ]]; then
  ...
  build_method="BuildScript.BuildWebGLCandidate"
elif [[ "$mode" == "reference" ]]; then
  build_method="BuildScript.BuildWebGLReferenceVisual"
else
