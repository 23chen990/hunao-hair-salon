import { createRandom, mixSeed } from './prng.ts';
import { DEFAULT_RULES } from './rules.ts';
import type { LevelDefinition, PieceDefinition } from './types.ts';

function generateCandidate(seed: number, generationAttempt: number): LevelDefinition {
...
export function generateLevel(seed: number, generationAttempt = 0): LevelDefinition {
  return generateCandidate(seed, generationAttempt);
}

const acceptedCache = new Map<number, LevelDefinition>();

export function generateAcceptedLevel(seed: number): LevelDefinition {
...
function evaluateForAcceptance(level: LevelDefinition): { accepted: boolean } {
...
function analyticSuccess(level: LevelDefinition, x: number, y: number, clickTime: number): boolean {
