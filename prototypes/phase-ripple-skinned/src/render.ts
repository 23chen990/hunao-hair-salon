...
import { resultActions } from './ui-actions.ts';
import type { GameState, Point } from './types.ts';
import type { TutorialPhase } from './onboarding.ts';

type Layout = ReturnType<typeof computeLayout>;

export type SkinAssets = Readonly<{
  board: HTMLImageElement;
  outboundPiece: HTMLImageElement;
  returnedPiece: HTMLImageElement;
}>;

export type GameViewMeta = Readonly<{
  progress: Readonly<{
    level: number;
    total: number;
    stage: number;
...
function copyFor(
  state: GameState,
  meta?: GameViewMeta,
): Readonly<{ title: string; detail: string; color: string }> {
...
function drawGoal(context: CanvasRenderingContext2D, center: Point, width: number): void {
  context.beginPath();
...
function drawResultButtons(
  ...
  width: number,
  height: number,
  tutorial?: GameViewMeta['tutorial'],
): void {
