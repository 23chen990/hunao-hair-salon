...
import type { ClickInput, LevelDefinition, SolutionSample, SolverOptions } from './types.ts';

export const DEFAULT_SOLVER_OPTIONS: SolverOptions = Object.freeze({
  columns: 24,
  rows: 40,
  timeStart: 0.35,
  timeEnd: 1.35,
...
function sampleCoordinate(minimum: number, maximum: number, index: number, count: number): number {
...
export function sampleSolutions(
  ...
  const heatmap = new Array<number>(sliceSize).fill(0);
  let successCount = 0;
  let firstSolution: ClickInput | null = null;
...
export function auditSeeds(count: number): AuditReport {
  let accepted = 0;
  let unsolved = 0;
  let nonContinuous = 0;
  let tooEasy = 0;
