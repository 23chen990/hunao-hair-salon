...
test('model-to-canvas mapping matches the production 390 by 844 layout', async () => {
  ...
  assert.deepEqual(modelToCanvas({ x: 0, y: 0 }, 390, 844), { x: 195, y: 417.78 });
  const mapped = modelToCanvas({ x: 37.68005381583613, y: -64.5807301973603 }, 390, 844);
  assert.ok(Math.abs(mapped.x - 237.05094005847312) < 1e-9);
  assert.ok(Math.abs(mapped.y - 345.70790509974586) < 1e-9);
});
